package com.adactin_hotel.crypto.services;

import com.adactin_hotel.crypto.config.CryptoAlgorithmTypes;
import com.adactin_hotel.crypto.config.CryptoKeyParameters;
...
import static com.adactin_hotel.crypto.utils.InputValidator.validateKeySize;
import static com.adactin_hotel.crypto.utils.InputValidator.validateSize;
...
public final class SecureKeyGenerator {
    ...
    public static byte[] generateIv() {
        try {
            return generateIv(CryptoKeyParameters.IV_SIZE.getKeySize());
    ...
    public static byte[] generateSalt() {
        try {
            return generateSalt(CryptoKeyParameters.SALT_SIZE.getKeySize());
        } catch (Exception error) {
    ...
    public static SecretKey generateSecretKey() {
        try {
            return generateSecretKey(CryptoKeyParameters.SECRET_KEY_SIZE.getKeySize());
        } catch (Exception error) {
    ...
    public static SecretKey generateSecretKey(int sizeInBytes) {
        try {
            ...
            return new SecretKeySpec(keyBytes, CryptoAlgorithmTypes.AES.getAlgorithmName());
        } catch (Exception error) {
            ErrorHandler.logError(error, "generateSecretKey", "Failed to generate secret key");
            throw error;
        }
